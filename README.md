...
### ระบบแจ้งเตือน
- **8:30** — รายงานงานค้างข้ามวัน (ส่งทุกวัน ทุกกลุ่ม)
- **12:00 / 16:00** — งานวันนี้ที่ยังเปิดอยู่ (ส่งเฉพาะกลุ่มที่มีงานค้าง)
- **17:30** — สรุปรายวัน: งานทั้งหมด แยก ปิดแล้ว/ยังไม่ปิด
...
| ปิดงาน | `ปิดงาน W001 วางกับดักหนู 2 ตัว` | `W001 ปิดแล้ว โดย คุณสมชาย ✅` |
...
### 2. รายงานงานค้างวันนี้ (`?type=check`) — 12:00, 16:00

ส่งเฉพาะกลุ่มที่มีงานเปิดอยู่วันนี้ (ถ้าไม่มี → ไม่ส่ง):

```
📋 งานที่ยังค้างอยู่ (2 งาน):
• W005 — ปลวก อาคาร B ชั้น 3 [เปิด]
• W006 — หนู โกดัง C [รับทราบ]
...
| `aga-notify-check` | `0 12,16 * * *` | `?type=check` |
